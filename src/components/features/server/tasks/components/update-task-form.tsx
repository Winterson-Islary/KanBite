...
const taskSchemaWithOmits = createTaskSchema.omit({
	workspaceId: true,
	description: true,
});

type TUpdateTaskFormProps = {
	onCancel?: () => void;
...
export default function UpdateTaskForm({
	...
	memberOptions,
	initialValues,
}: TUpdateTaskFormProps) {
	const workspaceId = useWorkspaceId();
	const router = useRouter();
	const { mutate, isPending } = useUpdateTask();
	const inputRef = useRef<HTMLInputElement>(null);

	const form = useForm<z.infer<typeof taskSchemaWithOmits>>({
		resolver: zodResolver(taskSchemaWithOmits),
		defaultValues: {
			...initialValues,
	...
	return (
		<Card className="h-full w-full border-none shadow-none">
			<CardHeader className="flex p-2 lg:p-0">
				<CardTitle className="font-normal text-3xl">Edit a task</CardTitle>
			</CardHeader>
